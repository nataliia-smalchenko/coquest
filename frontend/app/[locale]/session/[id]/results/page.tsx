...
import type { GameSessionDetailResponse, SessionProgress, SessionPlayer } from "@/types/session"
...
export default function ResultsPage() {
  ...
  const [results, setResults] = useState<GameSessionDetailResponse | null>(null)
  ...
  const loadResults = async (token: string) => {
    ...
    setError(null)
    try {
      const data = await getResults(sessionId, token) as GameSessionDetailResponse
      setResults(data)
      ...
      setError("Результати недоступні або токен невірний")
    } finally {
      setLoading(false)
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-lg p-8 w-full max-w-sm">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Введіть токен гравця</h2>
          <input
            value={tokenInput}
            onChange={(e) => setTokenInput(e.target.value)}
            placeholder="Ваш guest_token..."
  ...
  if (!results) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <p className="text-gray-500">{error ?? "Результати недоступні"}</p>
      </div>
    )
  }
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-white rounded-2xl shadow-sm p-5 text-center">
            <p className="text-2xl font-bold text-gray-900">
              {answeredProgress.filter((p) => (p.score ?? 0) >= 1).length}/{totalQuestions}
            </p>
            <p className="text-sm text-gray-500 mt-1">{t("correct")}</p>
          </div>
          {durationMin !== null && (
            <div className="bg-white rounded-2xl shadow-sm p-5 text-center">
              <p className="text-2xl font-bold text-gray-900">{durationMin} хв</p>
        ...
        {myProgress.length > 0 && (
          <div className="bg-white rounded-2xl shadow-sm divide-y divide-gray-100">
            {myProgress.map((p, i) => {
              ...
              const isWrong = p.status === "answered" && !isCorrect && !isPending
