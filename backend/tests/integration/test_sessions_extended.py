"""Extended integration tests for session routes not covered by test_sessions.py."""
import pytest
...
@pytest.mark.asyncio
async def test_get_session_by_code(client: AsyncClient, teacher_headers, db_quest):
...
@pytest.mark.asyncio
async def test_get_session_by_code_lowercase(client: AsyncClient, teacher_headers, db_quest):
    session = await _create_session(client, teacher_headers, db_quest.id)
...
@pytest.mark.asyncio
async def test_update_session_settings(client: AsyncClient, teacher_headers, db_quest):
    session = await _create_session(client, teacher_headers, db_quest.id)
    sid = session["id"]
...
async def test_update_settings_wrong_teacher(
    ...
    token = create_access_token({"sub": str(other.id), "email": other.email, "role": other.role})
...
@pytest.mark.asyncio
async def test_delete_session(client: AsyncClient, teacher_headers, db_quest):
    session = await _create_session(client, teacher_headers, db_quest.id)
    sid = session["id"]
...
@pytest.mark.asyncio
async def test_delete_active_session_blocked(client: AsyncClient, teacher_headers, db_quest):
    session = await _create_session(client, teacher_headers, db_quest.id)
    sid = session["id"]
...
async def test_get_game_info(client: AsyncClient, teacher_headers, db_quest):
    ...
    player = await _join_session(client, code)
...
async def test_update_player_guest_name(client: AsyncClient, teacher_headers, db_quest):
    ...
    player = await _join_session(client, code, guest_name="OldName")
    pid = player["id"]
...
@pytest.mark.asyncio
async def test_invalid_guest_token_rejected(client: AsyncClient, teacher_headers, db_quest):
    session = await _create_session(client, teacher_headers, db_quest.id)
    sid = session["id"]
    resp = await client.get(
        f"/api/runs/{sid}/my-progress",
...
@pytest.mark.asyncio
async def test_create_scheduled_session(client: AsyncClient, teacher_headers, db_quest):
