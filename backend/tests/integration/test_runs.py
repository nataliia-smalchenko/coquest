...
async def test_create_and_list_runs(
    client: AsyncClient, teacher_headers: dict, db_quest
):
    # Create the session
...
async def test_join_run_as_guest(client: AsyncClient, teacher_headers: dict, db_quest):
    ...
    join = await client.post(
        "/api/runs/join", json={"session_code": session_code, "guest_name": "TestGuest"}
    )
    ...
    list_resp = await client.get(
        "/api/runs/", headers={"Authorization": f"Bearer {player_data['guest_token']}"}
    )
...
async def test_rejoin_run(client: AsyncClient, teacher_headers: dict, db_quest):
    ...
    # Join
    join = await client.post(
        "/api/runs/join", json={"session_code": session_code, "guest_name": "Rejoiner"}
    )
    token = join.json()["guest_token"]

    # Rejoin
    rejoin = await client.post(
        "/api/runs/rejoin", json={"session_code": session_code, "guest_token": token}
    )
...
async def test_player_start_run(client: AsyncClient, teacher_headers: dict, db_quest):
    ...
    # Player joins
    join = await client.post(
        "/api/runs/join", json={"session_code": code, "guest_name": "P1"}
    )
    token = join.json()["guest_token"]
    ...
    p_start = await client.post(
        f"/api/runs/{session_id}/player-start", headers={"x-guest-token": token}
    )
...
async def test_teacher_monitor(client: AsyncClient, teacher_headers: dict, db_quest):
    ...
    monitor = await client.get(
        f"/api/runs/{session_id}/monitor", headers=teacher_headers
    )
