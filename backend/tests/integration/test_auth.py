...
def mock_email_service():
    with patch(
        ...
        with patch(
            "app.services.auth_service.EmailService.generate_verification_token",
...
async def test_verify_email_and_login(
    ...
    # Mock welcome email
    with patch(
        "app.services.auth_service.EmailService.send_welcome_email",
        new_callable=AsyncMock,
    ) as mock_welcome:
        # Verify email using the explicit token that we mocked
        verify_response = await client.post(
            "/api/auth/verify-email", json={"token": "mock_token"}
        )
...
async def test_get_me(
    ...
    access_token = await test_verify_email_and_login(
        client, db_session, mock_email_service
    )
...
async def test_google_auth_new_user(client: AsyncClient, db_session: AsyncSession):
    ...
    with patch(
        "app.routes.auth.OAuthService.verify_google_id_token", new_callable=AsyncMock
    ) as mock_verify:
        mock_verify.return_value = mock_google_info

        response = await client.post(
            "/api/auth/google",
            json={"credential": "fake_google_id_token", "role": "student"},
        )
