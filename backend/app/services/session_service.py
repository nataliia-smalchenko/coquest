...
from app.models.quest import Quest, QuestResource, QuestSettings
...
def _auto_score(question, answer: dict) -> Tuple[Optional[float], bool]:
    ...
    if q_type == "single":
        ...
        correct_ids = {str(opt["id"]) for opt in question.options if opt.get("is_correct")}
        if not correct_ids:
            return 0.0, False
...
async def _advance_queue(
    ...
    """Assign the next queued (map_object_id=NULL) progress item to the freed map object."""
    ...
    if queued:
        queued.map_object_id = map_object_id
    await db.flush()
...
class SessionService:
    ...
    async def create_session(
        ...
        quest_result = await db.execute(
            select(Quest).where(Quest.id == data.quest_id, Quest.teacher_id == teacher_id)
        )
        ...
        sess_status = SessionStatus.SCHEDULED if data.scheduled_at else SessionStatus.WAITING
        session = GameSession(
    ...
    async def join_session(
        ...
        active_count = sum(1 for p in session.players if p.status != PlayerStatus.FINISHED)
        ...
        avatar_color = random.choice(available) if available else random.choice(AVATAR_COLORS)
    ...
    async def _distribute_resources(db: AsyncSession, session: GameSession) -> None:
        quest_result = await db.execute(
            ...
            .options(
                ...
                selectinload(Quest.resources).selectinload(QuestResource.resource),
            )
        )
        ...
        show_all_texts = settings.show_all_texts if settings else False
        ...
        for player in players:
            for i, qr in enumerate(player_resources_map[player.id]):
                ...
                db.add(SessionProgress(
                    ...
                    map_object_id=map_obj.id if map_obj else None,
    ...
    async def submit_answer(
        ...
        if progress.resource and progress.resource.question:
            score, requires_review = _auto_score(progress.resource.question, data.answer)
        ...
        if progress.map_object_id:
            await _advance_queue(db, progress.session_id, player.id, progress.map_object_id)
    ...
    async def get_session_results(
        ...
        if not player.results_available_until:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Results not yet available"
            )
        ...
        result = await db.execute(
            ...
            .options(
                ...
                selectinload(GameSession.chat_messages).selectinload(SessionChat.player),
            )
        )
        ...
        return GameSessionDetailResponse(
            ...
            created_at=session.created_at,
            players=[_player_response(p) for p in session.players],
            progress=[SessionProgressResponse.model_validate(p) for p in session.progress],
    ...
    async def get_teacher_monitor(
        ...
        for player in session.players:
            ...
            players_progress.append(PlayerProgressSummary(
